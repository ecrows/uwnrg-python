...
class AStarNode:
    ...
    def updateScore(self, xDest, yDest):
        #score is set based on the node's current distance and the estimated distance heuristic
    ...
    def adjustDistance(self, directionIndex): 
        ...
        if directionIndex % 2 != 0:
            self.totalDistance += 1.5 #increment the traveled distance by sqrt(2), a.k.a ~1.5
        else:
            self.totalDistance += 1.0 #increment by a standard edge length, 1

			
	#estimates the distance between the current position and the destination using a standard
	#linear difference
